...
func (a *EnterpriseApiService) EnterpriseGetObjectGroups(ctx context.Context, data EnterpriseGetObjectGroups) ([]ObjectGroup, *http.Response, error) {

	localVarHttpMethod := strings.ToUpper("Post")
	localVarHeaderParams := make(map[string]string)
	localVarHeaderParams["Content-Type"] = "application/json"

	r, err := a.client.prepareRequest(ctx, "/enterprise/getObjectGroups", localVarHttpMethod, data, localVarHeaderParams)
	if err != nil {
		return nil, nil, err
	}
	...
	var localVarReturnValue []ObjectGroup
	err = a.client.decode(&localVarReturnValue, localVarBody, localVarHttpResponse.Header.Get("Content-Type"))
	if err != nil {
		return nil, localVarHttpResponse, err
	}
